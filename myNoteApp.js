...
const SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InZnZWxoc3R4d2Fyb3lzdHh0dnBpIiwicm9sZSI6ImFub24iLCJpYXQiOjE3MTg3NjM3NDAsImV4cCI6MjAzNDM0OTc0MH0.YOUR_ACTUAL_KEY_HERE"; // Keep your massive true key here!
const supabaseClient = window.supabase.createClient(SUPABASE_URL, SUPABASE_ANON_KEY);

const translations = {
    en: {
        appTitle: "My Notes App", notesColumn: "Notes", listsColumn: "Checklists",
        newNote: "New Note", newChecklist: "New Checklist", createNoteBtn: "Create Note",
        createListBtn: "Create List", placeholderNote: "Note Title...", placeholderList: "Checklist Title...",
        ...
        saveCard: "Save Content", deleteCard: "Delete Card", innerTodoPlaceholder: "Add item...",
        innerTodoBtn: "Add", listDone: "Done! 🎉", overlayTitle: "Create Workspace Item"
    },
    es: {
        appTitle: "Mi Aplicación de Notas", notesColumn: "Notas", listsColumn: "Listas de Tareas",
        newNote: "Nueva Nota", newChecklist: "Nueva Lista", createNoteBtn: "Crear Nota",
        createListBtn: "Crear Lista", placeholderNote: "Título de la Nota...", placeholderList: "Título de la Lista...",
        ...
        saveCard: "Guardar", deleteCard: "Eliminar", innerTodoPlaceholder: "Añadir tarea...",
        innerTodoBtn: "Añadir", listDone: "¡Hecho! 🎉", overlayTitle: "Crear Elemento de Trabajo"
    }
};
...
let currentUser = null; // Tracks active logged-in instance
...
// UI Interactivity Listeners
floatingAddBtn.addEventListener("click", () => {
    creationOverlay.classList.remove("hidden");
    setTimeout(() => checklistTitleInput.focus(), 300); 
});
closeOverlayBtn.addEventListener("click", () => creationOverlay.classList.add("hidden"));
...
async function handleSignUp() {
    ...
    try {
        ...
        alert("Account processing initialized! Check your email inbox for a confirmation link to activate your dashboard profile.");
    } catch (err) { authErrorMsg.textContent = err.message; }
}
...
async function handleLogIn() {
    ...
    try {
        ...
        if (data?.user) {
            ...
            authOverlay.classList.add("hidden"); // Remove auth screen block
            fetchNotesFromCloud();
        }
    } catch (err) { authErrorMsg.textContent = err.message; }
}
...
// Check if user is already logged in on session start
...
async function executeChecklistCreation() {
    ...
    const newListObj = { 
        ...
        name: titleText, 
        items: [],
        user_id: currentUser.id // STAMPING EXPLICIT USER OWNER TAG
    };
    try {
        const { error } = await supabaseClient.from('notes').insert([newListObj]);
        if (error) throw error;
        ...
        renderDashboard();
    } catch (err) { alert("Cloud write failure: " + err.message); }
}
...
async function executeNoteCreation() {
    ...
    try {
        const { error } = await supabaseClient.from('notes').insert([newNoteObj]);
        if (error) throw error;
...
checklistTitleInput.addEventListener("keydown", (e) => { if (e.key === "Enter") executeChecklistCreation(); });
createNoteBtn.addEventListener("click", executeNoteCreation);
noteTitleInput.addEventListener("keydown", (e) => { if (e.key === "Enter") executeNoteCreation(); });
...
async function fetchNotesFromCloud() {
    ...
    try {
        ...
        if (error) throw error;
        if (data) { dashboardState = data; renderDashboard(); }
    } catch (err) { console.error("Cloud Fetch Failure: ", err.message); }
}
...
function applyPalettePreferences() {
    ...
    if (activePalette === "default") {
        root.style.setProperty('--bg-color', '#f4f7f6'); root.style.setProperty('--surface-color', '#ffffff');
        root.style.setProperty('--text-color', '#333333'); root.style.setProperty('--primary-color', '#4caf50');
        root.style.setProperty('--primary-hover', '#45a049'); root.style.setProperty('--border-color', '#cccccc');
    } else if (activePalette === "ocean") {
        root.style.setProperty('--bg-color', '#04bdb3'); root.style.setProperty('--surface-color', '#004d40');
        root.style.setProperty('--text-color', '#f8f6f6'); root.style.setProperty('--primary-color', '#00bfa5');
        root.style.setProperty('--primary-hover', '#00b0ff'); root.style.setProperty('--border-color', '#00796b');
    } else if (activePalette === "sunset") {
        root.style.setProperty('--bg-color', '#fff3e0'); root.style.setProperty('--surface-color', '#3e2723');
        root.style.setProperty('--text-color', '#f38908'); root.style.setProperty('--primary-color', '#ff9800');
        root.style.setProperty('--primary-hover', '#f57c00'); root.style.setProperty('--border-color', '#e65100');
    } else if (activePalette === "cyberpunk") {
        root.style.setProperty('--bg-color', '#120136'); root.style.setProperty('--surface-color', '#03001e');
        root.style.setProperty('--text-color', '#00f5ff'); root.style.setProperty('--primary-color', '#ff007f');
        root.style.setProperty('--primary-hover', '#7300ff'); root.style.setProperty('--border-color', '#7300ff');
    }
}
...
function renderDashboard() {
    ...
    const langData = translations[currentLang];
    let noteCount = 0; let checklistCount = 0;
    ...
    if (noteCount === 0) {
        ...
        emptyNotice.style.color = "var(--text-color)"; emptyNotice.style.opacity = "0.6";
    ...
    for (let card of dashboardState) {
        ...
        if (!expandedCardsState[card.id]) cardBody.classList.add("collapsed");
        ...
        if (card.type === "note") {
            ...
            saveBtn.textContent = langData.saveCard; saveBtn.classList.add("save-card-btn");
            saveBtn.addEventListener("click", async () => {
                card.body_content = newText.value;
                saveBtn.style.backgroundColor = "var(--primary-hover)";
                try {
                    const { error } = await supabaseClient.from('notes').update({ body_content: card.body_content }).eq('id', card.id);
                    if (error) throw error;
                    setTimeout(() => { saveBtn.style.backgroundColor = "var(--primary-color)"; }, 500);
                } catch (err) { alert("Cloud save failed: " + err.message); }
            });
            const deleteCardBtn = document.createElement("button");
            deleteCardBtn.textContent = langData.deleteCard; deleteCardBtn.classList.add("delete-card-btn");
            deleteCardBtn.addEventListener("click", async () => {
                if(!confirm("Delete permanent?")) return;
                try {
                    const { error } = await supabaseClient.from('notes').delete().eq('id', card.id);
                    if (error) throw error;
                    dashboardState = dashboardState.filter(item => item.id !== card.id);
                    delete expandedCardsState[card.id];
                    renderDashboard();
                } catch (err) { alert("Cloud delete failed: " + err.message); }
            });
            actionsRow.appendChild(saveBtn); actionsRow.appendChild(deleteCardBtn);
            ...
            for (let task of card.items) {
                ...
                if (task.completed) { span.style.textDecoration = "line-through"; span.style.color = "gray"; }
                cb.addEventListener("change", async () => {
                    task.completed = cb.checked;
                    try {
                        const { error } = await supabaseClient.from('notes').update({ items: card.items }).eq('id', card.id);
                        if (error) throw error;
                        renderDashboard();
                    } catch (err) { alert("Task sync failed: " + err.message); }
                });
                li.appendChild(cb); li.appendChild(span); listWrapper.appendChild(li);
            }
            ...
            miniInput.type = "text"; miniInput.placeholder = langData.innerTodoPlaceholder;
            ...
            async function submitTaskItem() {
                ...
                try {
                    ...
                    if (error) throw error;
                    renderDashboard();
                } catch (err) { alert("Adding item failed: " + err.message); }
            }
            miniInput.addEventListener("keydown", (e) => { if (e.key === "Enter") submitTaskItem(); });
            const miniBtn = document.createElement("button");
            miniBtn.textContent = langData.innerTodoBtn; miniBtn.addEventListener("click", submitTaskItem);
            miniInputRow.appendChild(miniInput); miniInputRow.appendChild(miniBtn);
            cardBody.appendChild(listWrapper); cardBody.appendChild(miniInputRow);
...
// NEW: Real-time network listener restricted to current user records
supabaseClient
...
// Fire session detection check at startup
checkActiveSession();
